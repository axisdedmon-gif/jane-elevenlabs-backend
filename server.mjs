...
app.post("/api/travel/geocode", async (req, res) => {
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: data?.error?.message || "Geocoding failed." });
    const results = (data.features || []).map(feature => {
      const [lon, lat] = feature.geometry.coordinates;
      return { label: feature.properties.label || feature.properties.name || query, lat, lon };
    });
    res.json({ results });
  } catch (error) {
